...
class RegressionObjectives:
    ...
    def rulefit_regressor_objective(self, trial):
        param = {
            "tree_size": trial.suggest_int("tree_size", 2, 10),
            "max_rules": trial.suggest_int("max_rules", 100, 6000),
            "model_type": trial.suggest_categorical("model_type", ["r", "l", "rl"]),
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
        ...
        regressor = RuleFit(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def lasso_regressor_objective(self, trial):
        param = {
            "alpha": trial.suggest_loguniform("alpha", 1e-8, 1e-1),
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
            "tol": trial.suggest_loguniform("tol", 1e-8, 1e-1),
        ...
        regressor = Lasso(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def wrn_regressor_objective(self, trial):
        param = {}

    def dart_regressor_objective(self, trial):
        param = {
            "n_estimators": trial.suggest_int("n_estimators", 500, 4000),
            "max_depth": trial.suggest_int("max_depth", 8, 16),
            "min_child_weight": trial.suggest_int("min_child_weight", 1, 300),
    ...
    def nusvr_regressor_objective(self, trial):
        param = {
            "nu": trial.suggest_loguniform("nu", 1e-8, 1e-1),
            "C": trial.suggest_loguniform("C", 1e-8, 1e-1),
            "kernel": trial.suggest_categorical(
                "kernel", ["rbf", "linear", "poly", "sigmoid", "precomputed"]
            ),
            "degree": trial.suggest_int("degree", 1, 3),
            "gamma": trial.suggest_loguniform("gamma", 1e-8, 1e-1),
            "coef0": trial.suggest_loguniform("coef0", 1e-8, 1e-1),
            "shrinking": trial.suggest_categorical("shrinking", [True, False]),
            "tol": trial.suggest_loguniform("tol", 1e-8, 1e-1),
        }
        regressor = NuSVR(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def hgb_regressor_objective(self, trial):
        param = {
            "loss": trial.suggest_categorical(
                "loss", ["ls", "lad", "huber", "quantile"]
            ),
    ...
    def lars_regressor_objective(self, trial):
        param = {
            "eps": trial.suggest_loguniform("epsilon", 1e-8, 1e-1),
            "normalize": False,
            "random_state": self.random_state,
        }
        regressor = Lars(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def larscv_regressor_objective(self, trial):
        param = {
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
            "max_n_alphas": trial.suggest_int("max_n_alphas", 1, 2000),
            "eps": trial.suggest_loguniform("epsilon", 1e-8, 1e-1),
        ...
        regressor = LarsCV(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def tweedie_regressor_objective(self, trial):
        param = {
            "alpha": trial.suggest_int("alpha", 1, 20),
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
            "tol": trial.suggest_loguniform("tol", 1e-8, 1e-1),
        }
        regressor = TweedieRegressor(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def glm_regressor_objectives(self, trial):
        param = {
            "alpha": trial.suggest_int("alpha", 1, 20),
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
            "tol": trial.suggest_loguniform("tol", 1e-8, 1e-1),
        }
        regressor = GeneralizedLinearRegressor(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def ransac_regressor_objective(self, trial):
        param = {
            "max_trials": trial.suggest_int("max_trials", 100, 300),
            "stop_probability": trial.suggest_uniform("stop_probability", 0.1, 0.99),
        }
        regressor = RANSACRegressor(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def gpr_regressor_objective(self, trial):
        param = {
            "alpha": trial.suggest_float("alpha", 1e-10, 1e-2, log=True),
            "n_restarts_optimizer": trial.suggest_int("n_restarts_optimizer", 0, 5),
            "random_state": self.random_state,
        }
        regressor = GaussianProcessRegressor(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def omp_regressor_objective(self, trial):
        param = {
            "n_nonzero_coefs": trial.suggest_int("n_nonzero_coefs", 1, 20),
            "tol": trial.suggest_loguniform("tol", 1e-8, 1e-1),
            "normalize": False,
        }
        regressor = OrthogonalMatchingPursuit(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def ompcv_regressor_objective(self, trial):
        param = {
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
            "cv": self.cv,
            "normalize": False,
        ...
        regressor = OrthogonalMatchingPursuitCV(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    def llars_regressor_objectives(self, trial):
        param = {
            "alpha": trial.suggest_float("alpha", 1e-10, 1e-2, log=True),
            "max_iter": trial.suggest_int("max_iter", 1, 2000),
        ...
        regressor = LassoLars(**param)
        scores = cross_val_score(
            regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
        )
        return scores.mean()

    # def iso_regressor_objectives(self, trial):
    #     param = {
    #         "y_min": None,
    #         "y_max": None,
    #         "increasing": 'auto',
    #         "out_of_bounds": 'clip',
    #     }
    #     regressor = IsotonicRegression(**param)
    #     scores = cross_val_score(
    #         regressor, self.X, self.y, cv=self.cv, n_jobs=-1, scoring=self.metric
    #     )
    #     return scores.mean()
